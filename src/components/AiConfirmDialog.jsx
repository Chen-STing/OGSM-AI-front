...
const DARK = {
  bg:'#222222', text:'#F0F0F0', textSub:'#B4B4B4', border:'#DDDDDD', border_2:'#888888',
  inputBg:'rgba(255,255,255,0.09)', altBg:'rgba(255,255,255,0.12)', grid:'rgba(255,255,255,0.05)',
  scanline:'rgba(255,255,255,0.15)', backdrop:'rgba(0,0,0,0.88)', headerBg:'#111111',
}
const LIGHT = {
  bg:'#FFFFFF', text:'#111111', textSub:'rgba(0,0,0,0.60)', border:'#111111', border_2:'#444444',
  inputBg:'rgba(0,0,0,0.04)', altBg:'#F2F2F2', grid:'rgba(0,0,0,0.04)',
  scanline:'rgba(0,0,0,0.15)', backdrop:'rgba(255,255,255,0.85)', headerBg:'#0D0D0D',
}
...
export default function AiConfirmDialog({ type, currentText='', onConfirm, onCancel, darkMode=true, shapeConfig }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:T.backdrop, backdropFilter:'blur(4px)', zIndex:10300, display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.3s ease' }}
      ...
      `}</style>
      <div className="b-card animate-scale-in" role="dialog" aria-modal="true"
        style={{ background:T.bg, color:T.text, width:'520px', maxWidth:'92vw', maxHeight:'90vh', border:`4px solid ${T.border}`, boxShadow:`8px 8px 0px ${darkMode?'#223fce':'#7389dd'}`, position:'relative', overflow:'hidden', transition:'background 0.3s ease,color 0.3s ease,border 0.3s ease,box-shadow 0.3s ease', cursor:'default' }}>
        ...
        <div style={{ background:T.headerBg, padding:'12px 28px', display:'flex', justifyContent:'space-between', alignItems:'center', position:'relative', zIndex:10, transition:'background 0.3s ease' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <h2 style={{ fontFamily:'"Space Grotesk",sans-serif', fontWeight:900, fontSize:'20px', letterSpacing:'-0.03em', textTransform:'uppercase', color:'#F0F0F0', margin:0 }}>{config.headerLabel}</h2>
          </div>
          <button onClick={onCancel} style={{ background:'none', border:'none', color:'#F0F0F0', fontSize:'28px', lineHeight:1, transition:'color 0.15s', cursor:'pointer' }}
            onMouseEnter={e=>(e.currentTarget.style.color=ACCENT_PINK)} onMouseLeave={e=>(e.currentTarget.style.color='#F0F0F0')}>✕</button>
        </div>

        <div style={{ padding:'20px 28px', position:'relative', zIndex:10, overflowY:'auto', maxHeight:'calc(90vh - 70px)' }}>
          <p style={{ fontFamily:'"Space Grotesk",sans-serif', fontWeight:700, fontSize:'14px', lineHeight:1.5, color:T.textSub, marginBottom:'20px', marginTop:0, textTransform:'uppercase', transition:'color 0.3s ease' }}>{config.desc}</p>
          <label style={{ fontSize:'9px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:900, letterSpacing:'0.12em', textTransform:'uppercase', display:'block', marginBottom:'8px', color:T.text, transition:'color 0.3s ease' }}>{config.inputLabel}</label>
          <textarea style={{ width:'100%', padding:'14px 16px', fontSize:'15px', fontWeight:700, border:`4px solid ${T.border}`, fontFamily:'"Space Grotesk","Noto Sans TC",sans-serif', color:T.text, background:'transparent', backdropFilter:'blur(2px)', WebkitBackdropFilter:'blur(2px)', resize:'vertical', minHeight:'90px', outline:'none', lineHeight:1.6, transition:'background 0.3s ease,color 0.3s ease,border 0.3s ease' }}
            value={text} onChange={e=>setText(e.target.value)} placeholder={config.placeholder} rows={3} autoFocus />

          <div style={{ display:'flex', alignItems:'flex-start', gap:'10px', marginTop:'10px', marginBottom:'16px' }}>
            <span style={{ fontSize:'13px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:700, lineHeight:1.5, color:T.textSub, transition:'color 0.3s ease' }}>💡{config.hint}</span>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', gap:'12px', marginBottom:'8px' }}>
            ...
            <span style={{ fontSize:'13px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:700, lineHeight:1.5, color:T.text, transition:'color 0.3s ease' }}>定量指標 — 監控最終產出數字與績效</span>
          </div>
          <div style={{ display:'flex', alignItems:'flex-start', gap:'12px', marginBottom:'20px' }}>
            ...
            <span style={{ fontSize:'13px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:700, lineHeight:1.5, color:T.text, transition:'color 0.3s ease' }}>檢核步驟 — 每個定量指標底下的可執行行動</span>
          </div>

          {isEmpty && (
            <div style={{ marginBottom:'20px', padding:'14px 16px', border:`4px solid ${T.border_2}`, background:'#de993f', color:'#000' }}>
              <div style={{ fontFamily:'"Space Grotesk",sans-serif', fontWeight:900, fontSize:'13px', textTransform:'uppercase', letterSpacing:'0.06em' }}>⚠ 內容為空 — AI 將根據目前 OGSM 脈絡自行補充</div>
            </div>
          )}

          <div style={{ display:'flex', justifyContent:'flex-end', gap:'12px', marginTop:'4px' }}>
            <button onClick={onCancel} style={{ padding:'10px 20px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:900, fontSize:'15px', textTransform:'uppercase', background:T.bg, color:T.text, border:`4px solid ${T.border}`, boxShadow:shadowBase, transition:'all 0.15s', cursor:'pointer' }}
              onMouseEnter={cancelHover} onMouseLeave={cancelLeave} onMouseDown={cancelDown} onMouseUp={cancelUp}>取消</button>
            <button onClick={()=>onConfirm(text.trim())} style={{ padding:'10px 20px', fontFamily:'"Space Grotesk",sans-serif', fontWeight:900, fontSize:'15px', textTransform:'uppercase', background:ACCENT_YELLOW, color:'#000', border:`4px solid ${T.border}`, boxShadow:shadowBase, display:'flex', alignItems:'center', gap:'8px', transition:'all 0.15s', cursor:'pointer' }}
              onMouseEnter={confirmHover} onMouseLeave={confirmLeave} onMouseDown={confirmDown} onMouseUp={confirmUp}>{config.confirmLabel}</button>
          </div>
        </div>
      </div>
    </div>
