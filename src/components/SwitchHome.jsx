...
const LOCAL_CSS = `
  @keyframes waterfall { from { opacity: 0; transform: translateY(-30px); } to { opacity: 1; transform: translateY(0); } }
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ background: hovered ? "#000" : (dark ? "#fff" : "#000"), color: hovered ? ACCENT_YELLOW : (dark ? "#000" : "#fff"), fontSize: "9px", fontFamily: '"Space Grotesk", sans-serif', fontWeight: 900, letterSpacing: "0.1em", padding: "4px 8px", textTransform: "uppercase" }}>OGSM</div>
        {(isDone || isOverdue) && (
          <div style={{ fontSize: "9px", fontWeight: 900, padding: "4px 8px", background: isDone ? ACCENT_GREEN : ACCENT_PINK, color: "#000", textTransform: "uppercase", letterSpacing: "0.08em", border: "2px solid #000" }}>{isDone ? "完成" : "逾期"}</div>
        )}
      </div>
...
function NewProjectCard({ onNewProject, dark, index, size = 260 }) {
  ...
  return (
    <div onClick={onNewProject} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ width: `${size}px`, height: `${size}px`, flexShrink: 0, boxSizing: "border-box", border: `${borderW}px dashed ${dark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.3)"}`, boxShadow: hovered ? `8px 8px 0 0 ${dark ? "rgba(255,255,255,0.1)" : "#000"}` : "none", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: `${Math.round(16*scale)}px`, color: dark ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.4)", background: hovered ? ACCENT_BLUE : (dark ? "#333" : "#f5f5f5"), cursor: "pointer", transform: hovered ? "translate(-4px,-4px)" : "none", transition: "all 0.15s ease", animation: `waterfall 0.3s ${index * 0.04}s both` }}
    >
...
export default function ProjectsPage({ projects, onSelect, onNewProject, onBack, dark, onToggleDark, entering, exitingTo }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const diff = sortBy === "time" ? new Date(a.createdAt) - new Date(b.createdAt) : calcProgress(a) - calcProgress(b);
